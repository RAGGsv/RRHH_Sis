			...
			<form action="" id="manage-position">
				<div class="card">
					...
					<div class="card-footer">
						<div class="row">
							<div class="col-md-12">
								<button class="btn btn-sm btn-primary guardar"> Guardar</button>
								<button class="btn btn-sm btn-danger cancelar" type="button" onclick="_reset()"> Cancelar</button>
							</div>
						</div>
					</div>
				</div>
							...
							<tbody>
								...
								<tr>
									...
									<td class="text-center">
										...
										<button class="btn btn-sm btn-danger delete_position" type="button" data-id="<?php echo $row['id'] ?>">Borrar</button>
									</td>
								</tr>
								<?php endwhile; ?>
							</tbody>
	...
	$('#manage-position').submit(function(e){
		...
		$.ajax({
			...
			success:function(resp){
				if(resp==1){
					alert_toast("Actualizado con éxito",'success')
					setTimeout(function(){
						location.reload()
					},1500)
